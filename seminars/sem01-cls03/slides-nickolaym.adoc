...
=== Проблемы указателей на константы

(Дополнительные слайды по заданному на семинаре вопросу)

ifdef::backend-revealjs[=== !]

Можно (безопасно):

* привести *значение* типа `int*` к типу `const int*`

[source,cpp]
----
void f(const int* in) { cout << *in << endl; }
void g(int* in) { *in = 123; }

const int c = 42;
int v = 24;

const int* pcc = &c;
const int* pcv = &v;  // int* превратился в const int*
int* pv = &v;
f(pcc);
f(pcv);
f(pv);
...
Нельзя делать (не скомпилируется или будет расстрел):

* значение типа `const int*` к `int*`
...
g(pcv);  // НЕ СКОМПИЛИРУЕТСЯ
g(const_cast<int*>(pcc));  // РАССТРЕЛ
g(const_cast<int*>(pcv));  // +const, -const, ну ок.
----
...
* ссылку/указатель на `const int*` - к ссылке/указателю на `int*`
* наоборот - тоже нельзя

[source,cpp]
----
const int c = 42;

void h(const int*& outc, int*& outv) {
    outc = &c; // ок
    outv = const_cast<const int*>(&c);  // ЗАЛОЖИЛИ МИНУ
}

int* pv1;
int* pv2;
h(pv1, const_cast<int*&>(pv2));  // иначе не скомпилируется
*pv1 = 123;  // РАССТРЕЛ
*pv2 = 456;  // РАССТРЕЛ
----
